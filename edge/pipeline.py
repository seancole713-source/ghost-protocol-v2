...
def miss_review(get, ledger: Ledger, *, day: date, now: int, top: int = 50) -> Dict[str, Any]:
    store = ledger.store
    if now < _at(day, 16, 20):
        return {"status": "too_early"}
    ...
    minute = A.bars_multi(get, syms, timeframe="1Min", start=_iso(_at(day, 9, 30)), end=_iso(_at(day, 16, 0)))
    moves = []
    for s in syms:
        bs = daily.get(s) or []
    ...
    out = {"day": day.isoformat(), "movers": rep.movers, "executable": rep.executable,
           ...
           "coverage_note": "movers = Alpaca screener top-50 gainers, not the whole market"}
    store.put("edge_miss", day.isoformat(), out)
    return {"status": "reviewed", **{k: out[k] for k in ("movers", "executable", "caught", "recall", "labels")}}
