...
def log_research_prediction(
    *,
    ...
    Idempotent by (contract_sha, artifact_sha, symbol, issued_ts).
    ...
    if cur is not None:
        return _log_pred_impl(
            cur, contract_sha, artifact_sha, policy_lineage_id, symbol,
    ...
    with db_conn() as conn:
        ...
        pred_id = _log_pred_impl(
            c, contract_sha, artifact_sha, policy_lineage_id, symbol,
            direction, issued_ts, feature_available_ts, output,
            calibrated_prob, threshold, source_snapshot_sha,
            feature_snapshot_sha, selector_decision, context,
        )
...
def _log_pred_impl(
    ...
    cur.execute(
        ...
        WHERE contract_sha = %s AND artifact_sha = %s
          AND symbol = %s AND issued_ts = %s
        """,
        (contract_sha, artifact_sha, symbol, issued_ts),
    )
    ...
    if existing:
        ...
        if (existing_output != output
            ...
            raise ValueError(
                ...
                f"{contract_sha}/{artifact_sha}/{symbol}/{issued_ts}"
            )
    ...
    cur.execute(
        ...
        INSERT INTO ghost_research_predictions
            (contract_sha, artifact_sha, policy_lineage_id, symbol,
             direction, issued_ts, feature_available_ts, output,
             calibrated_prob, threshold,
             source_snapshot_sha, feature_snapshot_sha,
             selector_decision, context, created_at)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        ON CONFLICT (contract_sha, artifact_sha, symbol, issued_ts) DO NOTHING
        RETURNING id
        """,
        (
            contract_sha, artifact_sha, policy_lineage_id, symbol,
...
def get_pending_predictions(
    ...
    if cur is not None:
        return _get_pending_impl(cur, contract_sha, limit)
    from core.db import db_conn
    with db_conn() as conn:
        c = conn.cursor()
        return _get_pending_impl(c, contract_sha, limit)


def _get_pending_impl(cur, contract_sha, limit) -> List[Dict[str, Any]]:
    where = "WHERE r.prediction_id IS NULL"
    params: List[Any] = []
    if contract_sha:
        where += " AND p.contract_sha = %s"
        params.append(contract_sha)
    cur.execute(
        f"""
        SELECT p.id, p.contract_sha, p.artifact_sha, p.symbol, p.direction,
    ...
    return [
        {
            "id": r[0], "contract_sha": r[1], "artifact_sha": r[2],
...
def get_resolved_predictions(
    ...
    if cur is not None:
        return _get_resolved_impl(cur, contract_sha, artifact_sha, limit)
    from core.db import db_conn
    with db_conn() as conn:
        c = conn.cursor()
        return _get_resolved_impl(c, contract_sha, artifact_sha, limit)


def _get_resolved_impl(cur, contract_sha, artifact_sha, limit) -> List[Dict[str, Any]]:
