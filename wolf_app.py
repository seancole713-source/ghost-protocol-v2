...
def health():
    ...
    if feeds_ok < 2: issues.append(f"Price feeds: {feeds['summary']}")
...
def trigger_predictions(x_cron_secret: str = Header(default="")):
    """Manual trigger for prediction cycle. Used by cron-job.org."""
...
def trigger_reconcile(x_cron_secret: str = Header(default="")):
    """Manual trigger for outcome reconciliation."""
    if CRON_SECRET and x_cron_secret != CRON_SECRET:
        raise HTTPException(status_code=403, detail="Invalid cron secret")
...
def cockpit():
    ...
    return FileResponse("templates/cockpit.html")
