...
| **Last prod-verified** | **2026-06-29** — PR #105 deployed (`8234a16`, `_pr_version 105`); 582 tests passing; Top Pick Evidence Gate live (`/top-pick-gate`), `/picks` consumes backend gate and remains locked until direction+precision+calibration+expectancy+kill checks pass |
...
**PR #99 prod verify:** passed 2026-06-29 — `GET /api/_version` sha `cdf810b`, `_pr_version 99`; `/api/wolf/super-ghost/promotion?symbol=WOLF` ok (0 reviews cold-start, requirements present); `/promotion/review` 401 without auth; `/picks` includes Promotion gate Health row.
**PR #98 prod verify:** passed 2026-06-29 — `GET /api/_version` sha `3cc0987`, `_pr_version 97`; `/api/wolf/super-ghost/shadow?symbol=WOLF` ok with 7-model manifest; `/shadow/models` ok; `/shadow/run` and `/shadow/resolve` both 401 without auth; `/picks` includes Shadow models Health row.
**PR #96 prod verify:** passed 2026-06-29 — `GET /api/_version` sha `4ea2e24`, `_pr_version 96`; `/api/wolf/super-ghost/models` ok with production model; `/feature-profile?symbol=WOLF&horizon=5` ok (cold-start 0 profiles); `/features/score` 401 without auth; `/picks` includes Feature memory Health row.
**PR #95 prod verify:** passed 2026-06-29 — `GET /api/_version` sha `aba64f3`, `_pr_version 95`; `GET /api/wolf/super-ghost/lab?symbol=WOLF&horizon=5` ok (cold-start, 6 candidate manifest entries); `POST /api/wolf/super-ghost/lab/run` returns 401 without auth; `/picks` includes Research lab Health row.
...
| **#95** | 06-29 | **Champion/Challenger shadow lab** — `core/super_ghost_lab.py`, persistent lab runs/results, 6 candidate policies (production champion, coverage gate, strict confidence, grade B+, regime-aligned, edge-score), benchmark metrics, conservative recommendation gates, `/api/wolf/super-ghost/lab`, auth-gated `/lab/run`, console Health row; `_pr_version` 95 |
| **#96** | 06-29 | **Model registry + feature attribution memory** — `core/super_ghost_memory.py`, model/version registry, per-prediction checklist features, feature outcome scoring (helped/hurt/underweighted/noise/missing), feature reliability profiles, model contribution rows, `/models`, `/features`, `/feature-profile`, auth-gated `/features/score`, console Feature memory row; `_pr_version` 96 |
