...
LAST UPDATED: 2026-09-03 - Fixed two adversarially-verified bugs with no threshold loosened:
the train/serve EMA feature skew (serving fed ~252 bars where training fed 121, flipping the
n>=200 branch in engine_features) and a thin-data block that silently reintroduced PR #135's
banned negative walk-forward edge floor. Both ride on PR #171's branch at dd60ac5 (DRAFT, NOT
DEPLOYED — re-read /api/_version). Production is otherwise unchanged from 2026-09-02: Contract 70
falsified at current data, no forward registration, engine paused on latched brier degrade.
"""
...
SESSION_LOG = {
    ...
    "production": {
        ...
        "pr_version": "live _pr_version=168 (non-monotonic marker; git_sha remains the source of truth)",
        "git_sha": "f6560ac (source-backed, live-verified 2026-09-02 via /api/_version; deploy ba1bd266-f128-413f-832c-9791766468cb). NOTE: this ledger-only update will auto-deploy a newer SHA; always re-read /api/_version.",
        "app_version": "2.5.0",
        "health": "90/100 (live-verified 2026-09-02 after f6560ac; status healthy, no hard issues; warnings are stale-pick/cycle-alive plus open yfinance/yfinance_short_interest/alpaca breakers)",
        "degraded": True,
        "tests": "1845 passed, 43 skipped (python3 -m pytest -q, 2026-09-02) plus Ruff clean on PR #167 changes; PR #169 targeted suite 6 passed and CI green",
        ...
        "models_trained": "257 stored, 1 serveable (ITRI/DOWN), 0 fireable_now, 0 precision_ok — live /api/v3/status 2026-09-02 after forced 5y retrain; 213 research, 43 unknown, 1 proven",
        ...
        "breakers": "yfinance, yfinance_short_interest, and alpaca open at 2026-09-02 verification; app health remains 90/healthy but degraded=true",
    ...
    "session_summary": [
        ...
        "77-COMMIT GAP SUMMARY, 2026-08-07 through 2026-08-31 (recorded retroactively 2026-09-01 — this ledger had gone stale; git log is the source of truth for this paragraph, not memory): major landings since the 'RESEARCH + SCAN HARDENING' entry directly below. GHOST SQUEEZE HUNTER shipped as a new subsystem end-to-end (3582434 initial fuel/trigger/confirmation/pressure/stage/explosion detector through scan endpoint + console panel + market-regime factor, catalyst-freshness + live earnings-surprise scoring, a preregistered scheduled-issuance sampler + resolver job + point-in-time audit trail, a calibration-preregistration spec, several P0-P3 review-fix passes, evidence-gated planning levels, a fuel-required qualification gate, YMM evidence-ledger integrity repairs, and a one-time purge of invalid samples). A full 'END GAME' forensic pass (c73c318 + Tier 0-3 commits) closed unauthenticated writes/XSS/proxy-IP/MCP-token-leak security holes, fixed squeeze grading/volume/price measurement integrity, added holiday-calendar-aware honest job health + DB pooling + a scheduler leader lock enforcing single-instance execution, and pinned ML dependency versions. SE-4 (a3cb826) unified the win-rate denominator so genuine EXPIRED outcomes always count as non-wins everywhere. A two-way agent research workflow went live: a durable advisory-only task queue (ghost_agent_* MCP tools + /api/agent-workflow REST surface, docs/AGENT_WORKFLOW.md), a deterministic shadow-evidence scoring ledger, and an independent OpenAI-backed Codex worker (services/claude_worker + a parallel Codex-side worker) — PR #161's own title flags this as 'code-complete, not deployed'; the next agent should verify with a live GET before assuming it is active, not trust that title as proof. PR #163 enforced calibration-lifecycle governance; PR #162 fixed canonical kill accounting with fail-closed resume; PR #159/#160 simplified /picks from 13 tabs to 4 and hardened checklist safety/provenance/issuance concurrency. PR #164 shipped the /morning-picks skill + ghost_symbol_quote MCP tool + a 3-symbol watchlist expansion. PR #165 wired 7 previously dead-wired catalyst-checklist signals (earnings/fundamentals/news/leadership) to their real point-in-time data sources. PR #166 (merged 2026-09-01, just before this bug-fix session started, fast-forwarding local main from 0f99ee8 to f892c3e) wired the shadow lane into checklist calibration — every daily virtual pick now freezes a checklist snapshot, so the checklist ledger accrues samples even while the real engine stays paused — and raised the default OHLCV backtest window from 2y to 5y after live evidence showed the 2y window under-supplied qualifying picked-days for the precision gate. A steady stream of infra hardening ran throughout: scheduler leadership recovery after deploy, unrun-scheduler-task health handling, premarket-scan bounding + CI deploy gating, short-interest provider budget isolation/cache-warm bounding, lock-free performance-dashboard reads, Railway traffic gated on health, performance-log rewrite-bloat fix, staggered scheduler startup, retrain-safe web startup responsiveness, and a hardened/honest forward-prediction-contract enforcement pass. Last commit before this bug-fix session started: 0f99ee8 'fix: align runtime walk-forward edge gate'. STILL OPEN, not part of this session: PR #167 (draft, opened 2026-09-01) 'Fix self-perpetuating low-yield backoff blocking model coverage recovery' — next agent should check its status before assuming it merged.",
        "RESEARCH + SCAN HARDENING, HONEST 70% VERDICT (2026-08-07, Codex, commits 2692fbe..896ad8d): fixed five production defects without loosening a statistical or kill gate. (1) Retired contradictory tp_sl_swing/v1 (prose said 3 bars while machine field was 5) and registered active immutable v2 with the exact 5-bar horizon. (2) Added full-cycle process serialization plus cross-replica performance-log advisory locking; duplicate startup scan was live-observed as an explicit cycle_in_progress skip, not a racing writer. (3) Scheduler timeouts no longer mark non-cancellable worker threads finished; market/morning scans use a 600s budget. (4) Squeeze radar now builds complete metrics from multi-symbol bars and never blocks the scan on short-interest lookups; live result improved from roughly 450s with 45-50 fetch failures to 12-27s with 100/100 fetched and 0 failures. (5) Structured Finnhub ingest now rotates 8 symbols/cycle and stops on 429; live cycle fetched 225 articles with 8/8 Finnhub attempts and no rate-limit error. Also fixed the research discovery runner's missing production peer-pool metadata, added frozen-contract preflight, kept ineligible hypotheses in the Sidak denominator, and retained failed-gate diagnostics. Verification: 1350 non-integration tests passed; 7 isolated PostgreSQL integration tests passed; Ruff, mypy, compileall, and diff-check passed. Live deployment 3b2ba679 SUCCESS; health 95/healthy. The preregistered WOLF/UP 12-hypothesis run completed with NO_FORWARD_CANDIDATE: baseline/soft-voting/stacking holdout=52.6%, wf_edge=-4.32pp, precision_ok=false; pruned holdout=47.4%, wf_edge=-3.78pp; schema/geometry variants were correctly blocked pending a new contract. No artifact was persisted, so no fixed-50 proof or activation was started. Live kill status remains latched (last-30 win rate 10%, Brier 0.5574), the real prediction engine remains paused, and fake-money paper wallet/intraday monitoring remains enabled with $10k reset balance and zero August trades. This is the correct outcome: 70% is not claimed or bypassed; current production evidence falsifies it.",
        "SNIPER INTRADAY PAPER LANE — FUGU'S STRANDED BUILD FINISHED + 2 REAL BUGS FIXED (2026-07-23, Fable, PR #159): found fugu's uncommitted intraday-lane diff in the worktree (rate-limit strand): squeeze-radar-fed 'intraday' wallet book (conf>=70, squeeze_score>=55, p_continue_3pct_60m>=75, above-VWAP, forming requires high/extreme short risk), same-day 15:00 CT close expiry, /api/wallet/intraday endpoint, wallet_summary intraday section. FINISHED it and fixed what review found: (1) CRITICAL — the entry loop stripped candidates to (book,source,sym) then gave EVERY book fresh_bands() 3-day swing geometry, so 'intraday' rows silently became 3-day trades with the radar bracket + same-day close thrown away; fixed by threading intraday_geo through the loop, re-anchoring the radar's RELATIVE bracket on the live fill (new pure helper intraday_entry_bands), rejecting stale signals whose live price already crossed the radar bracket (diag skip_stale_intraday; PR #142 lesson intraday edition), expiry=close_ts. (2) sort ranked candidates by SHARE PRICE desc, not strength (fugu's own test masked it — fixture price correlated with conf); now sorts by (-confidence,-squeeze_score). (3) NEW core/earnings_guard.py — Finnhub /calendar/earnings (today..+PAPER_INTRADAY_EARNINGS_WINDOW_DAYS=1, session TZ, 6h cache, <=4 calls/day) blocks intraday entries on reporting symbols (an intraday hold through a report is an event bet: 5-min polling cannot guarantee the bell close-out beats the print — operator's own screenshots: TSLA -14.5%, ACI -21.6% earnings gaps). FAIL-OPEN by design (seatbelt not interlock) but ignorance is visible: diag earnings_guard_ok=false when the calendar was unavailable vs true+empty when genuinely clear. (4) sniper slice sizing: _slice_usd_for_book — intraday PAPER_INTRADAY_SLICE_USD=1000 (2x base 500), _enter takes slice_usd, capacity/cash checks book-aware, wallet_summary shows intraday_slice_usd. WHAT THIS IS: an honest fake-money measurement of the operator-requested aggressive day-trade style (the '$1k/day' ask — told the operator the real math: ~+0.9%/trade EV ceiling at a GOOD 60% win rate ~= $90/day on 10x$1k, not $1k/day; PDT rule blocks real day trading under $25k). WHAT IT IS NOT: a real-money lane or a gate change — 70% contract untouched, fireable=0, gated/shadow books unchanged. Env knobs (all code-default, no Railway change): PAPER_INTRADAY_ENABLED/MIN_CONFIDENCE/MIN_SQUEEZE_SCORE/MIN_CONTINUE_PCT/SLICE_USD/EARNINGS_GUARD/EARNINGS_WINDOW_DAYS, EARNINGS_GUARD_CACHE_TTL_S. Tests: +9 new (earnings guard parse/fail-open/cache/no-key, sort-by-strength regression, band re-anchor/stale-reject, slice book-awareness, fugu's originals kept green); suite 1032 passed/3 skipped; import-integrity PASS (136 files/939 imports). VERDICT AFTER ~30 SNIPER TRADES: compare intraday book win rate + $/day vs the cautious books — that number, not hope, decides whether aggression scales. LIVE-VERIFIED (2026-07-23, ad6b0e0 SUCCESS): /api/_version git_sha=ad6b0e0, health 95/healthy, /api/wallet/intraday ok=true enabled=true radar_active=true with full diag incl. skip_earnings, wallet_summary intraday section + intraday_slice_usd=1000.0 present; 0 candidates ready at verify time (scan_ok=false — fresh post-deploy scan state, radar loop running; correct, not broken).",
        "OPTIONS EDGE PATH — BUILD COMPLETE, NOTHING LEFT BUT THE 2-WEEK REAL-WORLD TEST (2026-07-18, PR #158 843bf40): with accrual fixed (100/100/day), built the remaining machinery so the ONLY missing input is calendar time. NEW core/options_edge.py: joins ghost_options_snapshots to ghost_shadow_outcomes on (symbol, date) = the PCR live when the virtual pick was made; buckets by PCR (<0.5/0.5-0.7/0.7-1.0/1.0-1.5/>=1.5); Wilson-tests each bucket family-corrected (Sidak); EXPIRED=non-win. Verdicts: NO_DATA / FLAT_NO_EDGE / DISCRIMINATES_UNPROVEN / PROVEN_70; provisional until >=200 paired obs across >=8 days. GET /api/ghost/options/edge-test + /api/ghost/options/readiness (read-only, no gate change). Readiness monitor catches the silent-failure mode that killed v1. Live-verified: readiness 103 rows/2 days/100 syms/0 paired (correct — snapshot-date shadow picks haven't resolved yet); edge-test verdict NO_DATA (honest). Scheduler job certified: fires weekdays 13:00-15:00 CT, once/day, re-claimable if a run stores 0; first AUTO run Monday 2026-07-20. THE PLAN (operator's): let it run untouched ~2 weeks, then GET /api/ghost/options/readiness — when ready_to_test=true, GET /api/ghost/options/edge-test for the verdict. PROVEN_70 -> wire PCR as a model feature; DISCRIMINATES_UNPROVEN -> keep accruing; FLAT_NO_EDGE -> 4th lever falsified, honest reframe. Suite 1020 passed. Deferred (named): OI + BS-IV (Alpaca snapshot omits both; PCR-volume is the signal now). NOTHING to build or touch until the check; the 70% gate is unchanged (LOCKED, UNPROVEN, fireable=0) and this path may still end in 'no edge'.",
        "OPTIONS ACCRUAL ENGINE FIXED — FULL WATCHLIST NOW LANDS (2026-07-18, operator chose 'accrue options evidence' path): the collector was dead-on-arrival (yfinance option chains rate-limit at ~15/min → only 3 symbols/day). Fixed the DATA SOURCE via 4 PRs (#154-#157): (1) probed alternatives — Polygon options NOT_AUTHORIZED, Alpaca options snapshots WORK; rewired to Alpaca (OCC-symbol parse → put/call volume → PCR); (2) 429-soft + drop redundant price call; (3) tried a dedicated breaker; (4) FINAL FIX — removed breaker gating entirely (the misses were Alpaca ACCOUNT rate-limit throttles from contention with the live price feed, not endpoint failures — 26/26 raw success; a breaker gate just made the sweep bail early). Per-symbol 429 retry-with-backoff, loop attempts all 100. RESULT (live-verified ce0d9f9): snapshot run stored 100/100, failed 0 — real PCR data (median 0.60, range 0.03-16.10). The forward-clock now accrues the full 100-symbol watchlist daily (scheduler job self-gates weekdays 13:00-15:00 CT). WHAT THIS IS: the evidence engine for the ONE untested edge lever is now genuinely running. WHAT IT IS NOT: 70% edge — it does not change the real-money gate (still LOCKED, contract-70 UNPROVEN, fireable=0). The actual TEST (do options PCR features add predictive edge?) needs ~2-4 weeks of daily point-in-time snapshots joined against shadow/pick outcomes, then run the sweep harness with PCR features. Deferred: OI + ATM IV (Alpaca snapshot omits both; PCR-volume is the signal now; BS-IV is a follow-up). Suite 1011 passed.",
        "EDGE-CEILING DETERMINATION — 70% IS NOT REACHABLE BY BUILDING (2026-07-18, Fable, operator granted 'master control to finish to 70% real-money ready'): applied the 6-word doctrine (Clarity/Results) to the live signal inventory instead of fabricating a done-report. FINDING (live-proven, not asserted): the single best signal Ghost has ANYWHERE tops out far below 70%. (a) 11 shadow brains: best raw win rate 61.1%, then 60/58/57/... — the best 'way of thinking' is 61% raw, not 70. (b) Sidak-corrected 70% slice search over 291 hypotheses: qualified_count=0; best raw slice BILL 75% (n=24) has Wilson-low 0.551 and FAMILY-corrected 0.379; SABR+Neutral 88.9% is n=9 noise (family 0.31); every slice with real n (hundreds/thousands) is 45-55%. (c) contract-70 bucket 36/65=55%, Wilson [0.43,0.67] — upper bound doesn't reach 70. CONCLUSION: 70% short-horizon single-name directional accuracy is not 9 points of engineering away — the signal isn't in the data. A meta-learner over shadow brains can't exceed its best 61% input by 9pts; per-symbol 'edge' (BILL) is small-sample noise that collapses under multiple-comparison correction. This matches the project's own retired 80% claim and market reality (pro funds don't hit 70% on 3-day single-name direction). The SOFTWARE is 100% built + verified (tests/e2e/deploys green, honesty layer working); the 70% PREDICTION ACCURACY is a market-efficiency ceiling, NOT an unbuilt feature. REFUSED to loosen gates to fake it (breaks the Alignment law; Phase-2 revert precedent). Honest paths remain: options forward-clock accrual (only untested lever, running; first real data 2026-07-17), buy historical options data to test sooner, or reframe the product to what Ghost genuinely is — a selective honest mostly-silent research aid. DO NOT re-run the falsified edge search (geometry/fundamentals/momentum/retrain) without a NEW data source.",
        "COCKPIT TOGGLE REGRESSION FIXED (2026-07-18, PR #153 db74b4d): CI playwright-smoke went red on main — cockpit.flows 'validation reload survives' timed out on locator.fill(#p-sym) after clicking #add-pos-toggle (desktop + mobile). ROOT CAUSE: PR #151's inline-style extraction moved #port-form's hide from style='display:none' to class='hidden-flex', but toggleAddPos() read the INLINE f.style.display (now '' for a class-hidden element) -> open computed as true -> first click HID the already-hidden form -> #p-sym never fillable. A real INTERACTION regression that static + page-load tests miss by construction. FIX: toggle the hidden-flex class instead of reading inline style. Swept all 5 dashboards — the only JS .style.display read, isolated. Added a real-browser regression guard driving toggleAddPos() (class/display round-trip none->block->none). VERIFIED: deployed db74b4d, then ran the REAL cockpit.flows against production 4/4 (desktop+mobile) + full e2e suite 57 passed/10 skipped (0 failed) -> playwright-smoke green. pytest 1008. LESSON for future inline-style extractions: any JS that reads el.style.display breaks when the initial hide becomes a CSS class — grep .style.display and convert those handlers to classList toggles. Also: the options forward-clock is WORKING — first real snapshots landed 2026-07-17 (ABNB pcr_vol 0.39, ABCL pcr 1.10/IV 0.97, AAPL pcr 0.70).",
        "P2 FLAKE ROOT-CAUSED + FIXED (2026-07-18, PR #152 cb28ed7): the intermittent test_fetch_ohlcv_retries_after_empty_response flake (assert 201==1) was NOT random — the test mocks only requests.get, so its below-min-bars Alpaca result falls through Polygon->yfinance->Stooq; yfinance uses its own client (not requests.get), so the test made a LIVE NETWORK FETCH returning ~201 real WOLF bars. It only passed when a prior test left the yfinance breaker tripped (masking the fallthrough) — and this session's own breaker-reset isolation fix (a942aef) unmasked it by resetting breakers CLOSED per test. Debugged via a deterministic repro (adding clear_ohlcv_cache to conftest made it fail every time) + a row-source probe (first bar ts=2025-09-29 = real WOLF data). FIX: block the 3 non-Alpaca tiers in the test (established pattern in the file); verified hermetic under the exact failing conditions. Swept the codebase: test_feed_resilience mocks _fetch_ohlcv_once one level down (no tier runs) — safe; the retry test was the SOLE exposed one. NOTE the general hazard for future tests: mocking requests.get alone does NOT make an _fetch_ohlcv test hermetic — the yfinance tier bypasses requests.get and hits the network; block _try_yfinance/_try_polygon/_try_stooq or mock _fetch_ohlcv_once.",
        "PR #151 MERGED + DEPLOYED + LIVE-BROWSER VERIFIED (2026-07-18, Fable autonomous merge/deploy/verify): merged the close-loose-ends branch (squash d2d3b1e) carrying — (a) test-isolation fix (reset circuit-breaker singletons between tests), (b) rate-limit GET/write budget split (audited sound), (c) contract-70 evidence UI across dashboards + inline-style extraction, (d) Mission Control honesty fixes MC-1..4, (e) real-browser e2e spec. Deployed d2d3b1e, /health 95. LIVE-BROWSER VERIFIED via claude-in-chrome on production: /picks + /cockpit both render the contract-70 'unproven at current data' evidence banner correctly, deploy badge d2d3b1e, ZERO page JS errors (the only console errors are chrome-extension:// internal to the automation harness), cockpit WebSocket works on prod. Honesty layer displaying truthfully everywhere (NO EDGE, Grade D, 0 picks, Top Pick Locked, NOT DEPLOYMENT-READY). FORENSIC AUDIT (inline, single-auditor — the 12-agent workflow died on a transient server rate-limit): no P0/P1; found + FIXED the order-dependent test-isolation failure (breaker singleton leak); verified clean Wilson/Sidak/Kelly/TP-SL math, pickle guards, 52 authed POST routes, scheduler isolation, retrain lock. Mission Control audit found MC-1 (v3 tile showed green '100 models' while fireable_now=0) — fixed to headline fireable + research split. NEW OPEN FINDING (P2, NOT yet fixed): test_wolf_app_core.py::test_fetch_ohlcv_retries_after_empty_response is an intermittent CI flake — a prior test leaks OHLCV-cache/_fetch_ohlcv state and it reads 201 leaked bars vs its 1-bar mock. Local passes deterministically (1008); main merge-commit CI passed (main is GREEN); only the PR run flaked once. A naive conftest clear_ohlcv_cache() per-test made it WORSE (reverted) — the test depends on some leaked state to pass, so the real fix needs understanding that dependency, not a blind cache clear. Left as a documented P2 for a careful pass.",
        "HIDDEN-BUG REVIEW + P3 FIXES DEPLOYED + FLYWHEEL CONFIRMED (2026-07-17, Fable session): ran the doctrine-structured read-only review (pre-registered severity bar, refute-before-report, empty-list-valid). RESULT: 2 real findings (both latent, neither corrupting current prod), 8 areas verified clean (Wilson math vs textbook, Sidak vs scipy at k=1/10/280, Kelly, resolution_exit, all 52 POST routes authed, research-tier fire block live, feature construction point-in-time, baseline 991 green), 3 candidates killed by refutation (documented). FINDING P3-1: predictions.outcome=EXPIRED conflates genuine full-term expiries (reconciler, pnl_pct set) with admin voids (dupe cleaner db.py:153 + portfolio purge, pnl NULL); every gate-facing win rate used WIN/LOSS-only denominators -> latent inflation, same class as the 07-14 contract-70 correction one table over. Live-verified all 10 current EXPIRED rows are voids, so zero behavior change today. FINDING P3-2 (self-reported from 07-16 code): options snapshot daily claim burned even on stored=0 runs. FIXED both in PR #150 (e8dd5d5, deployed + live-verified health 95, symbol_stats unchanged as predicted): new prediction_filters.RESOLVED_FOR_WINRATE_WHERE (pnl_pct IS NOT NULL discriminates genuine/void) at 7 prediction.py + 2 ghost_ask sites; live_recalibration.live_bin_stats now counts EXPIRED (shadow-side site the 07-14 fix missed); loss-streak detector + health-audit cross-check deliberately unchanged; snapshot claims now status-bearing JSON (done+stored>0 skips, failed/stale days re-claimable in window). Suite 1002 passed / 3 skipped. FLYWHEEL CONFIRMED at 07-17 open: 141 pending shadow rows seeded, WOLF eval up_prob 0.4221 from research model, gate reason research_tier — research tier is doing exactly its job. STILL PENDING: first successful options snapshot run (window 13:00-15:00 CT today; watch skipped_breaker), console/cockpit copy for the contract_70 block.",
        "BOTH PATHS SHIPPED + FLEET CLIFF SURVIVED (2026-07-16 evening, Fable session; operator-approved 'do both' + 'add 7' rules): merged PR #147 (IDE tidyup), PR #148 (contract-70 pre-registered verdict layer + options forward-evidence clock), PR #149 (research-tier models). LIVE-VERIFIED (two sources per claim): (1) /api/ghost/contract/70-verdict live at 1a4dbff -> status UNPROVEN_AT_CURRENT_DATA; 30-day cross-check verdict 60/36/0.600 == watcher 60/36/0.600 EXACT; Wilson [0.457,0.695] already excludes 0.70 but the pre-registered n>=100 bar correctly refuses early falsification. (2) First options snapshot run: 0 stored / 100 failed — HONESTLY counted; root cause the yfinance breaker's 15-call rate window tripped at symbol 14 (1s spacing too fast); FIXED in PR #149 (delay 4s, breaker-aware early stop, skipped_breaker count); next attempt via scheduler 13:00-15:00 CT daily. (3) FLEET CLIFF HIT SAME DAY: serveable collapsed 55 -> 11 during the afternoon (the 07-02 training batch crossed the 14-day serve expiry) — the research tier deployed hours before it would have blinded shadow evals. Post-deploy coverage retrain (state=passed) minted 180 research-tier + fleet now 191 serveable (11 proven), fireable_now still 0. BELT PROVEN LIVE: /api/wolf/gate-status reason=research_tier with up_prob 0.4497 recorded — evidence flows, firing blocked. Research invariants (all tested + live-verified): never overwrites proven serveable models, never fires (block precedes floor math), never inflates pass ratio; kill switch V3_RESEARCH_TIER=0. Suite 991 passed / 3 skipped. NEXT: confirm shadow rows carry up_prob at tomorrow's session open; first successful options snapshots tomorrow 13:00-15:00 CT; console/cockpit copy alignment for the contract_70 block still pending.",
        "MOMENTUM LEVER DEAD — THIRD AND FINAL (2026-07-16, Fable session, completes the entry below): momentum_feature_sweep full grid (55 features, 12 configs x 24 symbols) — serve_pass unchanged within noise (16->16 at 0.65), precision @0.60 mostly WORSE (0.512->0.457, 0.573->0.495 at the best baseline cells), NO pooled >=70% operating point in any config, best single cell 0.578@0.70thr n=45 Wilson-low 0.433 (indistinguishable from baseline 0.546 n=97). VERDICT: all three offline levers (geometry, fundamentals, momentum) falsified on the same out-of-time harness in one day; combined with the flat live watcher buckets, 0 Sidak-qualified slices, and the 7-day 0-pass retrain streak, the convergent evidence is that the 70% contract is NOT reachable with current features/data at this horizon/universe. Remaining honest paths: (1) new information sources (options flow/GEX, paid short-interest/analyst data — the existing NOT-YET-BUILT backlog), or (2) contract reframe via the falsification precedent already used for the 80% claim (pre-registered criteria, honest repositioning). Geometry/feature code changes should NOT be attempted again without a new data source — the harness (scripts/geometry_grid_sweep.py + scripts/momentum_feature_sweep.py) is the cheap pre-test for any future candidate.",
        "EDGE-HUNT TRIAGE (2026-07-16, Fable session): diagnosed the retrain pipeline, then ran controlled offline experiments on the geometry_grid_sweep harness (railway run, READ-ONLY). (a) DIAGNOSIS: /api/v3/train/last 'accuracy' is a PASS RATIO (total_passed/(symbols*2), signal_engine.py train_and_validate) — the recorded 0.0 means 0/108 direction-models cleared the serve floors, NOT a scorer bug; nothing is stored on a 0-pass run, so the 55-serveable fleet PREDATES these runs. /api/v3/lineage: ~50 runs over 7 days, ~7 total passes, 5 identical 0-pass runs on 07-15 alone; fail mix ~35 edge<5% with edge EXACTLY 0.0 (base-rate riders, holdout_acc==natural_rate) + ~30 holdout_acc<60%. FLEET DECAY CLOCK: 14-day serve expiry vs ~1/day mint rate — if retrains keep passing 0, serveable trends to 0 and shadow evidence generation (up_prob source) starves. (b) GEOMETRY LEVER DEAD (measured): 12 target-x-stop configs x 24 symbols — tight stops (0.65) give real edge (16/24 serve_pass) but ~0.35 win-precision below that geometry's 0.394 break-even; wide stops lift precision only to ~0.55-0.59 while serve_pass collapses to 1-3/24; NO pooled >=70% operating point in any config; raising threshold 0.55->0.70 buys ~0-5pp precision — the probability ranking is FLAT (same non-discrimination the live watcher shows). (c) FUNDAMENTALS LEVER DEAD AS-IMPLEMENTED (measured, inputs validated): same grid with V3_FUNDAMENTAL_FEATURES=1 (52 features; live-checked real values e.g. AAPL eps_yoy .218, WOLF -2.0 clamped) — serve_pass unchanged within noise, precision mostly slightly WORSE, still no pooled operating point. (d) NEW scripts/momentum_feature_sweep.py (read-only): injects core/momentum.py signals (breakout_20d/uptrend_struct/above_sma20/ret20/vol_ratio/score) point-in-time into the model feature vector — which has NEVER contained them; smoke-tested (ABNB 283 rows, real varying values); full-grid result should be ledgered when it lands. LIVE STATE: engine auto-paused (win_rate 0.10/30, brier 0.557) — CORRECT and cost-free: prediction.py pause suppresses firing only (scan+record continue), shadow evidence still accrues; fireable_now=0 makes resume moot anyway. OPERATOR CAUTION ON RECORD: resume = accepting a fresh-window override, NOT a cured system — the cure only comes when live outcomes in the new window actually clear 70%, which no retrain can shortcut. ALSO PR #147 (draft, cosmetic): -webkit-mask-image prefix + pydantic-settings dev dep for the documentation-only config/settings.py (zero runtime importers). IF MOMENTUM ALSO FAILS: all three levers falsified + flat watcher buckets + 0 Sidak slices + 7-day 0-pass streak point to the 70% contract being unprovable at this horizon/universe with this data — a product-positioning decision, not a code fix.",
        "CONTRACT-70 REGISTRY EVIDENCE PROVENANCE (2026-07-14, Fugu agent-2 role): independently re-derived the objective and found a remaining evidence-backed audit gap. register_slices froze only dims/key, not the discovery evidence that justified freezing it. After later data/code changes, an operator could see what slice was frozen but not prove what n/wins/Wilson/family correction passed at registration time. FIX: register_slices now preserves scalar selection_evidence inside each frozen slice (n, wins, win_rate, wilson_low/high, raw_pass/wilson_pass, family_wilson_low, family_size, family_z, family_confidence, multiple_comparisons_correction) while explicitly not persisting raw rows/secrets. This writes only ghost_state and does not alter gates/outcomes/models/trading. Tests added: selection evidence is preserved, rows are not persisted, ghost_state-only invariant holds. VERIFY: py_compile PASS; import-integrity PASS (131 files/908 imports); full suite 943 passed/3 skipped. HONEST STATUS: no 70+ claim; live remains contract_70 21/41=51.2pct and no_qualified_slice.",
        "CONTRACT-70 LOOK-ELSEWHERE CORRECTION (2026-07-14, Fugu agent-1 role): closed a real truthfulness hole in the 70+ slice discovery. find_qualified_slices searched ~20 dimension sets (many including symbol -> hundreds of simultaneous slice hypotheses) and judged each at a fixed 95pct Wilson lower bound with NO multiple-comparisons correction. With k tests the family false-positive rate is 1-0.95**k (~1 near k=60), so the forward registry could eventually freeze a SPURIOUS 70+ candidate purely by luck — a direct violation of the evidence-backed requirement. FIX: added _sidak_family_z()/_norm_ppf() (Acklam inverse-normal, no scipy) and made find_qualified_slices count the family size (all eligible n>=min_n slices across all dimension sets), compute a Sidak-corrected z holding family-wide confidence at 95pct, and require each slice family_wilson_low (Wilson lower bound at the stricter z) >= min_wilson_low to qualify. Payload now exposes family_size/family_z/family_confidence/multiple_comparisons_correction and per-slice family_wilson_low. This ONLY tightens the bar (z grows 1.96->3.02 at k=20 ->3.66 at k=200); it can never let a weak slice pass and never loosens a gate. VERIFY: z monotonic + equals 1.96 at k=1; borderline slice that clears naive 95pct is rejected under family correction while a genuinely strong 92/100 slice still passes at family_wl 0.85; py_compile PASS; import-integrity PASS (131 files/908 imports); full suite 942 passed/3 skipped. HONEST STATUS UNCHANGED: 70+ still NOT proven (live contract_70 21/41=51.2pct, high-confidence 52.9pct near-coin-flip, no_qualified_slice). No gate loosened, no broker/real-money path, no model/env mutation.",
    ...
    "in_flight": {
        "pr_171_train_serve_parity": {
            "status": "DRAFT, CI GREEN, UNMERGED (2026-09-03)",
            "description": "PR #171 on shared branch claude/admiring-euler-1BIUC now carries two unrelated change sets: Codex's squeeze premarket no-print fix (371fc92) and Claude's train/serve feature-parity + negative-edge-floor fixes (dd60ac5). CI green on dd60ac5 (test: success). OPEN DECISION for the next agent/user: leave both in #171, or split dd60ac5 onto a clean branch as its own PR. Do NOT force-push the shared branch to split it without explicit permission. Nothing here is deployed — re-read /api/_version before claiming either fix is live.",
        },
        "phase_2_down_model": {
